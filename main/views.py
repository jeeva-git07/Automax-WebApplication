...
def main_view(request):
    return render(request,"views/main.html",{"name":"PAGE"})
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if listing_form.is_valid and location_form.is_valid:
                listing_form.save()
                location_form.save()
            messages.info(request,f"Your listing {id} is updated succesfully")
            return redirect("home") 
        else:
        ...
        print(e)
        messages.error(request,f"Error Occured while acess edit page")
        return redirect("home")
...
def inquire_listing_using_email(request,id):
    ...
    try:
        ...
        email_message=f"Hi {listing.seller.user.username}, {request.user.username} is intersted in your {listing.model}"
        send_mail(email_subject,email_message,'jeeva@gmail.com',[listing.seller.user.email, ],fail_silently=True)
        return JsonResponse({
            'success':True,
        })
    except Exception as e:
